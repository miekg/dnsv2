...
func TestWalkForward(t *testing.T) {
	...
	err := m.Walk(WalkForward, func(s Section, rr RR, i int) error {
		switch s {
		case Qd:
			if i == 0 && RRToType(rr) != TypeA {
				t.Errorf("expected type %s for %d RR in section %s, got %s", TypeA, 0, s, RRToType(rr))
			}
			if i > 0 {
				t.Errorf("too many (%d > 1) RRs in section %s", i, s)
			}
		case An:
			if i == 0 && RRToType(rr) != TypeCNAME {
				t.Errorf("expected type %s for %d RR in section %s, got %s", TypeCNAME, 0, s, RRToType(rr))
			}
			if i == 1 && RRToType(rr) != TypeA {
				t.Errorf("expected type %s for %d RR in section %s, got %s", TypeA, 1, s, RRToType(rr))
			}
			if i > 1 {
				t.Errorf("too many (%d > 2) RRs in section %s", i, s)
			}
		case Ar:
			if i == 0 && RRToType(rr) != TypeOPT {
				t.Errorf("expected type %s for %d RR in section %s, got %s", TypeOPT, 0, s, RRToType(rr))
				if i > 0 {
					t.Errorf("too many (%d > 1) RRs in section %s", i, s)
				}
			}
...
func TestWalkBackward(t *testing.T) {
	...
	err := m.Walk(WalkBackward, func(s Section, rr RR, i int) error {
		if j == 0 && RRToType(rr) != TypeOPT {
			t.Errorf("expected first RR type to be %s, got %s", TypeOPT, RRToType(rr))
		}
		j++
		return nil
	})

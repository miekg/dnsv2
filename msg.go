...
func id() uint16 {
	var output uint16
	if err := binary.Read(rand.Reader, binary.BigEndian, &output); err != nil {
...
func PackDomainName(s string, msg []byte, off int, compression map[string]uint16) (off1 int, err error) {
	return packDomainName(s, msg, off, compression)
}

func packDomainName(s string, msg []byte, off int, compression map[string]uint16) (off1 int, err error) {
	// XXX: A logical copy of this function exists in IsDomainName and should be kept in sync with this function.

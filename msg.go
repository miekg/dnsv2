...
type (
	...
	Msg struct {
		...
		r       [3]uint16 // indices of section starts (0 means no such section), this is updated as we read from the message.
...
func (m *Msg) RR(s Section) (RR, error) {
	...
	if m.r[s] == 0 {
		m.r[s] = m.skipToSection(s)
	}
	i := m.skipName(m.r[s] + 1)
	if i == 0 {
...
func (m *Msg) skipRR(offset int) int {
	// must be at beginning of RR. If begining of Question offset = 13
	// question section is dealt with seperately.
	i := m.skipName(offset)
	if offset == 12 {
